...
export default function CourseDocuments({ setLoading, user }) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => (
                <Button
                    type="link"
                    onClick={() => handleShowDocuments(record.courseClassId)}
                >
                    Chi tiết
                </Button>
            ),
    ...
    const handleUnregister = async (courseClassId) => {
        try {
            await axios.delete(`http://localhost:5148/api/course-classroom/user/${user.name}/${courseClassId}`)
...
<Modal
        ...
        >
            {documents.length > 0 ? (
                documents.map((doc) => (
                    <div key={doc.documentId} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
