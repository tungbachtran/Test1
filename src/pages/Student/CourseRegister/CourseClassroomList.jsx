...
export default function CourseClassList({ setLoading, user }) {
    ...
    const handleRegister = async (courseClassId) => {
        try {
            await axios.post(`http://localhost:5148/api/course-classroom/user/${user.name}/${courseClassId}`) //user.name = userId
            setModal({
                isShow: true,
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            try {
                const { data: course } = await axios.get(`http://localhost:5148/api/course/${courseId}`)
