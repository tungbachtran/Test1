...
export default function Score({ setLoading, user }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
          ...
          let data = [];
          try {
            const { data: scoreData } = await axios.get(`http://localhost:5148/api/score/student/${user.name}`);
    
            // Combine fetching course details and filtering in a single loop
            for (const item of scoreData) {
              const { data: course } = await axios.get(`http://localhost:5148/api/course/${item.courseClassroom.courseId}`);
