...
export default function CreateCourse({ setLoading }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const { data: facultiesData } = await axios.get('http://localhost:5148/api/faculty/')
        ...
        try {
            ...
            for (let session of schedule) {
                try {
                    ...
                    const response = await axios.post('http://localhost:5148/api/schedules', session)
                } catch (err) {
                    // Xử lý lỗi ở đây nếu việc đăng lịch không thành công
                    await axios.delete(`http://localhost:5148/api/course-classroom/${e.courseClassId}`)
                    setModal({
                        isShow: true,
                        Fn: () => setModal({ ...modal, isShow: false }),
                        isDanger: true,
    ...
    const handleSelectFaculty = async (facultyId) => {
        setLoading(true)
        try {
            const { data: teachersData } = await axios.get(`http://localhost:5148/api/faculty/users/${facultyId}`)
