            ...
            try {
                const messagesWithNames = await Promise.all(
                    messages.map(async (message) => {
                        const { data } = await axios.get(`http://localhost:5148/api/user/${message.senderId}`);
                        return {
                            ...message,
        ...
        connect.start()
            .then(() => {
                ...
                connect.on("ReceiveMessage", (user, message, timestamp) => {
                    ...
                    if (user !== info) {  // "info" là ID người gửi (client)
                        ...
                        setMessagesWithSenderNames(prevMessages => {
                            ...
                            if (!prevMessages.some(msg => msg.content === message && msg.senderName === user)) {
