...
export default function CourseClassDocuments({ user, setLoading }) {
    ...
    const fetchTitleId = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:5148/api/course-classroom/${courseClassId}`);
    ...
    const handleAdd = async () => {
        ...
        try {
            await axios.post(`http://localhost:5148/api/documents/${courseClassId}?Content=${values.name}`, formData);
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'action',
            render: (_, document) => (
                <Button type="link" danger onClick={() => handleDelete(document.documentId)}>Xóa</Button>
            ),
        },
    ];
